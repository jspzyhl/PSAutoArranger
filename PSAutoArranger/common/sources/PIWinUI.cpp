...
int PIDialog::Modal(SPPluginRef plugin, const char *, int ID, HWND* owner_hwnd)
{
	dialogID = ID;
	pluginRef = plugin;
	...
	itemHit = (int)(DialogBoxParam(GetDLLInstance(pluginRef),
		...
		owner_hwnd ? *owner_hwnd : GetActiveWindow(),
		(DLGPROC)GenericProc,
		(LPARAM)this));
	...
	LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		if (owner != NULL)
			owner->Notify(item);
		if ((item == 1 || item == 2) && cmd == BN_CLICKED)		//	确定、取消按钮导致关闭对话框
			EndDialog(hDlg, item);
		return TRUE;
		break;
	}
...
void PIStaticText::SetText(const string & in)
{
	SendMessage(item, WM_SETTEXT, (WPARAM)0, (LPARAM)(LPCTSTR)in.c_str());
}

void PIStaticText::GetText(string & out)
{
	int strLen = (INT)SendMessage(item, WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0);
	if (strLen == 0) return;
	strLen++;
...
int32 PIComboBox::GetUserData(int32 index)
{
	return (int32)(SendMessage(item, CB_GETITEMDATA, index, 0));
}

void PIComboBox::SetUserData(int32 index, int32 data)
{
	SendMessage(item, CB_SETITEMDATA, index, data);
}

void PIComboBox::GetCurrentData(int32 & out)
{
	int index = (INT)SendMessage(item, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
	if (index == CB_ERR) return;
	out = (int32)(SendMessage(item, CB_GETITEMDATA, index, 0));
}

void PIComboBox::GetCurrentIndex(int32& out)
{
	out = (INT)SendMessage(item, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
}

void PIComboBox::GetText(std::string& out)
{
	HWND hEB = GetWindow(item, GW_CHILD);
	int strLen = (INT)SendMessage(hEB, WM_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0);
	if (strLen == 0) return;
	strLen++;
	vector<char> v(strLen + 1);
	SendMessage(hEB, WM_GETTEXT, (WPARAM)strLen, (LPARAM)(LPCSTR)&v[0]);
	out.assign(v.begin(), v.begin() + strLen);
}

...
public:
	...
	virtual int Modal(SPPluginRef plugin, const char * name, int ID, HWND* owner_hwnd = nullptr);
};
...
public:
	...
	void GetCurrentIndex(int32& out);
	...
	{
		EnableWindow(item, iEnabled);
	}
	void SetReadOnly(bool iReadOnly)
	{
		HWND hEB = GetWindow(item, GW_CHILD);
		SendMessage(hEB, EM_SETREADONLY, (WPARAM)iReadOnly, (LPARAM)0);
	}
	void SetText(const std::string& str)
	{
		HWND hEB = GetWindow(item, GW_CHILD);
		SendMessage(hEB, WM_SETTEXT, (WPARAM)0, (LPARAM)(LPCTSTR)str.c_str());
	}
	void GetText(std::string& out);
...
public:
	...
	{
		EnableWindow(item, iEnabled);
		//SendMessage(item, WM_ENABLE, (WPARAM)iEnabled, (LPARAM)0);
	}
	void SetReadOnly(bool iReadOnly)
	{
		SendMessage(item, EM_SETREADONLY, (WPARAM)iReadOnly, (LPARAM)0);
	}
...
class PIStaticText {
	PIItem item;
public:
	PIStaticText() : item(NULL) {}
	PIStaticText(PIItem in) : item(in) {}
	~PIStaticText() {}
